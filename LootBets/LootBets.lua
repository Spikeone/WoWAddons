function LB_OnLoad()
    ...
    DEFAULT_CHAT_FRAME:AddMessage("Loot Bets Addon loaded!");

    SLASH_LOOTBETS1 = "/lb";
    SlashCmdList["LOOTBETS"] = function(msg)
        LB_SlashCommandHandler(msg);
    end
end

function LB_SlashCommandHandler(msg)

    LB_createDonateFrame()
    donateFrame:Show()
...
function LB_OnEvent(self, event, ...)

end

function LB_BuidFrame()
    LB_InfoFrame = CreateFrame("Frame");
    LB_InfoFrame:SetHeight(300);
    LB_InfoFrame:SetWidth(600);
    LB_InfoFrame:SetBackdrop({bgFile="Interface/DialogFrame/UI-DialogBox-Background", edgeFile="Interface/DialogFrame/UI-DialogBox-Border", tile="true"});
    LB_InfoFrame:SetBackdropColor(0,0,0,1);
    LB_InfoFrame:SetAlpha(1);
    LB_InfoFrame:Show();
end

function LB_createDonateFrame()
    ...
    checkbox = CreateFrame("CheckButton", "Auc_Util_Appraiser_ShowAuctions", donateFrame, "OptionsCheckButtonTemplate")
    --checkbox:SetScript("OnEnter", function() return frame.SetButtonTooltip("Include own auctions\nin the item listing") end)
    --checkbox:SetScript("OnLeave", function() return GameTooltip:Hide() end)
    checkbox:SetWidth(24)
    checkbox:SetHeight(24)
    checkbox:SetPoint("BOTTOM", 5, 0)
