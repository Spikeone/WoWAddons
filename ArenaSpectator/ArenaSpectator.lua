...
local createAuraIcon = function(unit, framename, icons, index)
	local button = getglobal("aura"..framename..unit..index)
	
	if(not button) then
		button = CreateFrame("Button", "aura"..framename..unit..index, icons)
		...
		button:SetParent(icons)
	
		-- just to make sure, not needed probably
		button:SetWidth(icons.size or 24)
		button:SetHeight(icons.size or 24)
...
local function Reset()
    if (players ~= nil) then
        for _, p in pairs(players) do
			...
			players[p.name].fsmall.Debuffs = nil
			players[p.name].ftarget.Debuffs = nil
			players[p.name].fself.Debuffs = nil
...
local function KnockBackSpell(target, knockback)
        for _, barname in pairs(ALLBARS) do
            if knockback > 0 then
                local old = players[target][barname].cast:GetValue()
                local new = old - knockback / 1000
                if (new < 0) then
                    new = 0
                end
                
                players[target][barname].cast:SetValue(new)
            else
                local old = players[target][barname].cast:GetValue()
                local new = old + knockback / 1000
                if (new < 0) then
...
local function Execute(target, prefix, ...)
    ...
    elseif (prefix == "SPB") then
        local knockack = tonumber(strsub(value, strfind(value, ",") + 1))
        KnockBackSpell(target, knockack)
...
local function EventHandler(self, event, ...)
    ...
    if(true) then
        if (event == "CHAT_MSG_ADDON") then
            if ((arg1 == "ARENASPEC") and (arg3 == "WHISPER")) then
                ParseCommands(arg2)
            end
