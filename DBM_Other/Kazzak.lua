...
function Kazzak:OnEvent(event, arg1)
	...
	or event == "CHAT_MSG_SPELL_PERIODIC_FRIENDLYPLAYER_DAMAGE" then
		if string.find(arg1, DBM_KAZZAK_TWISTED) then
			local _, _, target, isSelf = string.find(arg1, DBM_KAZZAK_TWISTED);
			if target == DBM_YOU and isSelf == DBM_ARE then
				target = UnitName("player");
			end
			
			if target and self.Options.TwistedWarn then
				self:Announce(string.format(DBM_KAZZAK_TWISTED_WARN, target))
			end
			
		elseif string.find(arg1, DBM_KAZZAK_MARK) then
			local _, _, target, isSelf = string.find(arg1, DBM_KAZZAK_MARK);
			if target == DBM_YOU and isSelf == DBM_ARE then
				target = UnitName("player");
			...
			elseif self.Options.Whisper and DBM.Rank >= 1 and target then
			...
			if target and self.Options.Icon then
				DBM.SetIconByName(target);
				self:ScheduleSelf(8, "ClearIcon", target);
			end
		end
