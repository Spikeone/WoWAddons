...
function WeakAuras.SetAuraVisibility(
  id,
  triggernum,
  cloneId,
  inverse,
      ...
      for triggernum, data in pairs(triggers) do
        if (not data.specificUnit or UnitIsUnit(data.unit, unit)) then
          ...
          if (data.fullscan) then
            ...
            while (name) do
              ...
              if
                (name and ((not data.count) or data.count(count)) and
                  ...
                  data.scanFunc(name, tooltip, isStealable, spellId, debuffClass))
               then
            ...
            -- Reset clone list
            if groupcloneToUpdate then
              wipe(groupcloneToUpdate)
            end
            ...
            for index, checkname in pairs(data.names) do
              ...
              if (checkPassed) then
                ...
                if (aura_object and not data.specificUnit) then
                  ...
                  local changed =
                    aura_object:AssertAura(id, uGUID, duration, expirationTime, name, icon, count, casGUID, spellId)
                ...
                if (satisfies_count) then
                  ...
                  if (data.groupclone) then
                    ...
                    for guid, playerName in pairs(groupcloneToUpdate) do
                      local duration, expirationTime, name, icon, count, spellId =
                        aura_object:GetPlayerDynamicInfo(id, guid, data)
                      if (name ~= "") then
                        if
                          (WeakAuras.SetAuraVisibility(
                            id,
...
do
  ...
  local function updateRegion(id, data, triggernum, GUID)
    ...
    if (auradata and auradata.unitName) then
      ...
      local icon =
        auradata.icon or WeakAuras.GetTempIconCache(auradata.name) or "Interface\\Icons\\INV_Misc_QuestionMark"
      if (state.icon ~= icon) then
        state.icon = icon
        state.changed = true
      end
    ...
    for id, triggers in pairs(loaded_auras[spellName]) do
      ...
      for triggernum, data in pairs(triggers) do
        ...
        local name,
          ...
          isStealable,
          shouldConsolidate,
          spellId = UnitAura(unit, spellName, nil, filter)
  ...
  local function combatLog(
    ...
    if (loaded_auras[spellName]) then
      if
        (message == "SPELL_AURA_APPLIED" or message == "SPELL_AURA_REFRESH" or message == "SPELL_AURA_APPLIED_DOSE" or
          message == "SPELL_AURA_REMOVED_DOSE")
       then
          ...
          for id, triggers in pairs(auras) do
            ...
            for triggernum, data in pairs(triggers) do
              ...
              local name,
                ...
                isStealable,
                shouldConsolidate,
                spellId = UnitAura(uid, spellName, nil, filter)
              if (name) then
                data.GUIDs = data.GUIDs or {}
...
frame:SetScript(
  "OnEvent",
  function(frame, event, arg1, arg2, ...)
    ...
    elseif (event == "UNIT_AURA") then
      if
        (loaded_auras[arg1] or (loaded_auras["group"] and (arg1:sub(0, 4) == "raid" or arg1:sub(0, 5) == "party")) or
          (loaded_auras["boss"] and arg1:sub(0, 4) == "boss"))
       then
            ...
            if
              (((not trigger.use_name) or
                (trigger.name and trigger.name ~= "" and
                  (trigger.name_operator == "==" and name == trigger.name or
                    trigger.name_operator == "find('%s')" and name:find(trigger.name) or
                    trigger.name_operator == "match('%s')" and name:match(trigger.name)))) and
                ((not trigger.use_tooltip) or
                  (trigger.tooltip and trigger.tooltip ~= "" and
                    (trigger.tooltip_operator == "==" and tooltip == trigger.tooltip or
                      trigger.tooltip_operator == "find('%s')" and tooltip:find(trigger.tooltip) or
                      trigger.tooltip_operator == "match('%s')" and tooltip:match(trigger.tooltip)))) and
                ((not trigger.use_stealable) or isStealable) and
                ((not trigger.use_spellId) or spellId == tonumber(trigger.spellId)) and
                ((not trigger.use_debuffClass) or debuffClass == trigger.debuffClass))
             then
              return true
            else
              return false
...
function BuffTrigger.Modernize(data)
  ...
  for triggernum = 0, (data.numTriggers or 9) do
    ...
    if (trigger and (trigger.count) and not tonumber(trigger.count)) then
      trigger.count = 0
    end
...
function BuffTrigger.CanHaveClones(data, triggernum)
  ...
  return (trigger.fullscan and trigger.autoclone) or (trigger.unit == "group" and trigger.groupclone) or
    (trigger.unit == "multi")
end
        ...
        for subgroup, players in pairs(groupMembers) do
          ...
          for playerName, _ in pairs(players) do
            ...
            playersString =
              playersString ..
              (classColor or "") ..
                (space and playerName:sub(0, space - 1) .. "*" or playerName) ..
                  (classColor and "|r" or "") .. (next(players, playerName) and ", " or "")
          end
        ...
        for playerName, _ in pairs(playerList) do
          ...
          playersString =
            ...
            (classColor or "") ..
              (space and playerName:sub(0, space - 1) .. "*" or playerName) ..
                (classColor and "|r" or "") ..
                  (next(playerList, playerName) and (", " .. (num % 5 == 4 and "\n" or "")) or "")
          num = num + 1
        end
        GameTooltip:AddLine(playersString)
      end
