...
function WeakAuras.AddOption(id, data)
  ...
  if(regionOptions[data.regionType]) then
    ...
    regionOption = {
      unsupported = {
        type = "description",
        name = string.format(L["This region of type \"%s\" has no configuration options."], data.regionType)
      }
    };
  end
...
function WeakAuras.ReloadTriggerOptions(data)
  ...
  local function getAuraMatchesLabel(name)
    ...
    if(ids) then
      ...
      if(numMatches == 1) then
        ...
        return string.format(L["%i Matches"], numMatches);
      end
    else
      return "";
    end
  ...
  local aura_options = {
    ...
    group_count = {
      ...
      name = L["Count"],
      desc = function()
        return L["Group aura count description"];
      end,
      order = 47,
      width = "half",
      hidden = function() return not (trigger.type == "aura" and trigger.unit == "group"); end,
  ...
  local trigger_options = {
    ...
    chooseTrigger = {
      ...
      order = 1,
      values = function()
        local ret = {[0] = string.format(L["Trigger %d"], 1)};
        if(data.controlledChildren) then
          for index=1,(data.numTriggers or 9) do
            ...
            if not(none) then
              if(all) then
                ret[index] = string.format(L["Trigger %d"], index + 1);
              elseif(any) then
                ret[index] = "|cFF777777"..string.format(L["Trigger %d"], index + 1);
              end
            end
          end
    ...
    triggerHeader = {
      ...
      name = function(info)
        if(info == "default") then
          ...
          if(optionTriggerChoices[id] == 0) then
            return string.format(L["Trigger %d"], 1);
          else
            return string.format(L["Trigger %d"], optionTriggerChoices[id] + 1);
          end
        end
      end,
      order = 2
...
function WeakAuras.CreateFrame()
  ...
  closebutton:SetScript("OnEnter", function(self)
      ...
      GameTooltip:AddLine("Closes WeakAuras, make sure to save your changes to any of your auras first!", 1, 1, 1)
      GameTooltip:Show()
  end)
  ...
  minimizebutton:SetScript("OnEnter", function(self)
      ...
      GameTooltip:AddLine("Minimizes WeakAuras temporarily.", 1, 1, 1)
      GameTooltip:Show()
  end)
  ...
  if(enabled and loadable) then
    ...
    tutorial:SetPoint("TOPRIGHT", -135, 12)
    ...
    tutorialbutton:SetNormalTexture("Interface\\Buttons\\UI-Panel-Button-Up");
    tutorialbutton:GetNormalTexture():ClearAllPoints();
    tutorialbutton:GetNormalTexture():SetHeight(16);
    tutorialbutton:GetNormalTexture():SetWidth(16);
    tutorialbutton:GetNormalTexture():SetPoint("center", -2, 0);
    tutorialbutton:SetPushedTexture("Interface\\Buttons\\UI-Panel-Button-Down");
    tutorialbutton:GetPushedTexture():ClearAllPoints();
    tutorialbutton:GetPushedTexture():SetHeight(16);
    tutorialbutton:GetPushedTexture():SetWidth(16);
    tutorialbutton:GetPushedTexture():SetPoint("center", -2, 0);
    ...
    tutorialbutton:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("Learn how to use WeakAuras without minimizing!", 1, 1, 1)
        GameTooltip:Show()
    end)
    tutorialbutton:SetScript("OnLeave", function (self)
        GameTooltip:Hide();
    end)
  ...
  function importexport.Open(self, mode, id)
    ...
    if(mode == "export" or mode == "table") then
      if(id) then
        ...
        importexportbox.editBox:SetFocus();
      end
  ...
  function texteditor.CancelClose(self)
    ...
    texteditorbox.editBox:ClearFocus();
    self.frame:Hide();
    frame.container.frame:Show();
    frame.buttonsContainer.frame:Show();
    frame.window = "default";
