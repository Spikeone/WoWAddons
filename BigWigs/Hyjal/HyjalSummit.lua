...
mod.revision = tonumber(match("$Revision: 80486 $", "%d+"))
	...
	elseif sync == "HSWave" and rest then
		...
		if wave and wave > currentWave and waveTimes[wave] then
			...
			if self.db.profile.detail then
				...
				if nextBoss == winterchill or nextBoss == L["Boss"] then
					if wave == 1 then
				...
				elseif nextBoss == kazrogal then
					...
					elseif wave == 5 then
						self:Message(fmt(three, wave, 2, ghoul, 4, necro, 6, fiend), "Important")
					elseif wave == 6 then
