...
L:RegisterTranslations("enUS", function() return {
	...
	normal_phase_trigger = "%s punches the ground in anger!",
	...
	punch_desc = "Warnt, wenn Shazzrah Glühender Hieb benutzt und zeigt einen Countdown an.",
	punch_message = "Shazzrah Fluch!",
	punch_bar = "~Möglicher Glühender Hieb!",

	curse = "Shazzrah Fluch",
    curse_desc = "Warnt, wenn Shazzrah Fluch benutzt und zeigt einen Countdown an.",
	curse_message = "Shazzrah Fluch!",
    curse_bar = "~Möglicher Fluch!",

    counterspell = "Shazzrah Gegenzauber",
    counterspell_desc = "Warnt, wenn Shazzrah Fluch benutzt und zeigt einen Countdown an.",
	counterspell_message = "Gegenzauber!",
    counterspell_bar = "~Möglicher Gegenzauber!",
    counterspell_urgent = "Zauber abbrechen!",

    teleport = "Shazzrah Teleport",
    teleport_desc = "Warnt, wenn Shazzrah Teleport benutzt und zeigt einen Countdown an.",
	teleport_message = "Teleport!",
    teleport_bar = "Teleport Countdown!",
    teleport_urgent = "Teleport gleich!",

    arcaneexplosion = "Shazzrah AOE",
    arcaneexplosion_desc = "Warnt, wenn Shazzrah AOE benutzt und zeigt einen Countdown an.",
	arcaneexplosion_message = "AOE!",
    arcaneexplosion_bar = "~Möglicher AOE",
...
L:RegisterTranslations("esES", function() return {
	...
	normal_phase_trigger = "¡Shazzrah golpea el suelo enfadado!",
...
L:RegisterTranslations("deDE", function() return {
	phase = "Phasen",
	phase_desc = "Warnung wenn Shazzrah zwischen Tank und Kitephase wechselt.",
	normal_phase_message = "Tank'n'spank!",
	normal_phase_trigger = "%s schlägt wütend auf den Boden!",
...
L:RegisterTranslations("frFR", function() return {
	...
	normal_phase_trigger = "De rage, Shazzrah frappe le sol !",
	...
	punch_desc = "Prévient quand Shazzrah utilise son Punch de la fournaise, et affiche une barre de cooldown.",
...
mod.zonename = BZ["Molten Core"]
mod.enabletrigger = boss
mod.guid = 12264
mod.toggleoptions = {"curse", "counterspell", "teleport", "arcaneexplosion", "bosskill"}
--mod.toggleoptions = {"punch", "target", "icon", "phase", "curse", "enrage", "bosskill"}
--mod.toggleoptions = {"counterspell", "teleport", "curse", "arcane explosion", "bosskill"}
...
function mod:OnEnable()
    ...
    self:AddCombatListener("SPELL_CAST_SUCCESS", "ArcaneExplosion", 19712)
    self:AddCombatListener("SPELL_CAST_SUCCESS", "Curse", 19713)
    self:AddCombatListener("SPELL_CAST_SUCCESS", "CounterSpell", 19715)
    self:AddCombatListener("SPELL_CAST_SUCCESS", "Teleport", 23138)
...
function mod:ArcaneExplosion(_, spellID)
	if db.arcaneexplosion then
		self:IfMessage(L["arcaneexplosion_message"], "Attention", spellID)
		self:Bar(L["arcaneexplosion_bar"], 5, spellID)
	end
end
...
function mod:Curse(_, spellID)
	if db.curse then
		...
		self:Bar(L["curse_bar"], 20, spellID)
	end
end

function mod:CounterSpell(_, spellID)
	if db.counterspell then
		self:IfMessage(L["counterspell_message"], "Attention", spellID)
        self:DelayedMessage(13, L["counterspell_urgent"], "Attention")
		self:Bar(L["counterspell_bar"], 15, spellID)
	end
end

function mod:Teleport(_, spellID)
	if db.teleport then
		self:IfMessage(L["teleport_message"], "Attention", spellID)
		self:Bar(L["teleport_bar"], 45, spellID)
        self:DelayedMessage(40, L["teleport_urgent"], "Attention")
	end
end
...
function mod:CHAT_MSG_RAID_BOSS_EMOTE(msg)
	if msg == L["normal_phase_trigger"] then
		...
		if db.target then
			self:CancelScheduledEvent("BWShazzrahToTScan")
	...
	elseif msg == L["kite_phase_trigger"] then
		...
		if db.target then
			self:ScheduleRepeatingEvent("BWShazzrahToTScan", self.TargetCheck, 1, self)
		end
	else
		self:Message("CHAT_MSG_RAID_BOSS_EMOTE", "Positive")
		self:Message(msg, "Positive")
	end
end
...
function mod:BigWigs_RecvSync(sync, rest, nick)
	if self:ValidateEngageSync(sync, rest) and not started then
		...
		if db.phase then
            self:Bar(L["counterspell_bar"], 15, 19715)
            self:Bar(L["curse_bar"], 10, 19713)
            self:Bar(L["teleport_bar"], 30, 23138)
            self:Bar(L["arcaneexplosion_bar"], 6, 19712)
			--self:DelayedMessage(15, L["counterspell_bar"], "Attention")
		end
		--if db.enrage then
		--	self:Enrage(900)
		--end
