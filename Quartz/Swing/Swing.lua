...
local WF_PROC = GetSpellInfo(25584)
local SS_PROC = GetSpellInfo(16459)
...
do
	...
	function QuartzSwing:COMBAT_LOG_EVENT_UNFILTERED(timestamp, event, srcGUID, srcName, srcFlags, dstName, dstGUID, dstFlags, ...)
		if (event == "SPELL_EXTRA_ATTACKS") and ((select(2, ...) == SS_PROC) or (select(2, ...) == WF_PROC)) and (bit.band(srcFlags, COMBATLOG_FILTER_ME) == COMBATLOG_FILTER_ME) then
			swordspecproc = true
