...
local MAJOR, MINOR = "LibTalentQuery-1.0", 90000 + tonumber(("$Rev: 57 $"):match("(%d+)"))
...
function lib:CheckInspectQueue()
	...
	if (self.lastQueuedInspectReceived and self.lastQueuedInspectReceived < GetTime() - 300) then
		-- No queued results received for 5 minutes, so purge the queue as invalid and move on with our lives
...
function lib:INSPECT_TALENT_READY()
	...
	if (self.lastInspectName and self.lastInspectPending == 0) then
		...
		if (inspectQueue[self.lastInspectName]) then
			...
			self.events:Fire("TalentQuery_Ready_Outsider", name, realm)
		end
