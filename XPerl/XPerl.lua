...
local function DoRangeCheck(unit, opt)
	...
	if (range == 0 and opt.PlusDebuff) then
		local name = UnitDebuff(unit, 1, 1)
		if (not name) then
			range = 0
		else
...
function XPerl_StartupSpellRange()
	...
	local function Setup1(self)
		if ((type(self.spell) ~= "string" and type(self.item) ~= "string") or
						(self.item and not GetItemInfo(self.item))) then
...
function XPerl_AllBarTextures(short)
	...
	local list = {
		...
		{"Perl", "Interface\\Addons\\XPerl\\Images\\XPerl_StatusBar"},
	}

	for i = 1,9 do
		tinsert(list, {"X-Perl "..i, "Interface\\Addons\\XPerl\\Images\\XPerl_StatusBar"..(i + 1)})
	end

	if (not short and LibStub) then
...
function XPerl_GetBarTexture()
	return (conf.bar.texture and conf.bar.texture[2]) or "Interface\\TargetingFrame\\UI-StatusBar"
end
...
-- UnitIsFeignDeath - chinese might not have patch yet.
if (type(UnitIsFeignDeath) ~= "function") then
	function UnitIsFeignDeath(unit)
		for i = 1,20 do
			local name, rank, buff = UnitBuff(unit, i)
...
function XPerl_ColourHealthBar(self, healthPct, partyid)
	...
	if (conf.colour.classbar and UnitIsPlayer(partyid)) then
		...
		if (class) then
			local c = barColours[class]	-- RAID_CLASS_COLORS[class]
			if (c) then
	...
	if (bar.text) then
		...
		if (self.conf.healerMode.enable and self.conf.healerMode.type ~= 2) then
			if (self.conf.healerMode.type == 1) then
				if (Max > 100000) then
					hbt:SetFormattedText("%d%s/%d%s", (hp - Max) / largeNumDiv, largeNumTag, Max / largeNumDiv, largeNumTag)
				else
					hbt:SetFormattedText("%d/%d", hp - Max, Max)
				end
			else
				if (Max > 100000) then
					hbt:SetFormattedText("%d%s", (hp - Max) / largeNumDiv, largeNumTag)
				else
					hbt:SetFormattedText("%d", hp - Max)
				end
			end
		else
			if (Max > 100000) then
				hbt:SetFormattedText("%.1f%s/%.1f%s", hp / largeNumDiv, largeNumTag, Max / largeNumDiv, largeNumTag)
			else
				hbt:SetFormattedText("%d/%d", hp, Max)
			end
		end
	end
...
function XPerl_MinimapMenu_Initialize(level)
	local info
...
function XPerl_MinimapMenu(self)
	...
	ToggleDropDownMenu(1, nil, XPerl_Minimap_Dropdown, self:GetName(), 0, 0)
end
...
function XPerl_MinimapButton_OnEnter(self)
	...
	GameTooltip:SetText(XPerl_Version.." "..XPerl_GetRevision(), 1, 1, 1)
	GameTooltip:AddLine(XPERL_MINIMAP_HELP1)
	GameTooltip:AddLine(XPERL_MINIMAP_HELP2)
	if (UpdateAddOnMemoryUsage) then
		if (IsAltKeyDown()) then
			GameTooltip:AddLine(XPERL_MINIMAP_HELP6)
		elseif (not IsShiftKeyDown()) then
			GameTooltip:AddLine(XPERL_MINIMAP_HELP5)
		end
	end
	...
	if (GetRealNumRaidMembers) then
		if (GetNumRaidMembers() > 0 and GetRealNumRaidMembers() > 0) then
			if (select(2, IsInInstance()) == "pvp") then
				GameTooltip:AddLine(format(XPERL_MINIMAP_HELP3, GetRealNumRaidMembers(), GetRealNumPartyMembers()))

				if (IsRealPartyLeader()) then
					GameTooltip:AddLine(XPERL_MINIMAP_HELP4)
				end
			end
		end
	end
	...
	if (UpdateAddOnMemoryUsage and IsAltKeyDown()) then
		...
		for k,v in pairs(xpModList) do
			...
			if ((usedKB or 0) > 0) then
				...
				if (showDiff) then
					...
					diffKB = diffKB + diff
					GameTooltip:AddDoubleLine(format(" %s", v), format("%.1fkb (%s%.1fkb|r)%s", usedKB, DiffColour(diff / 500), diff, cpuText), 1, 1, 0.5, 1, 1, 1)
				else
					GameTooltip:AddDoubleLine(format(" %s", v), format("%.1fkb%s", usedKB, cpuText), 1, 1, 0.5, 1, 1, 1)
				end
			end
		end
		...
		if (showDiff) then
			...
			GameTooltip:AddDoubleLine("Total", format("%.1fkb (%s%.1fkb|r)", totalKB, color, diffKB), 1, 1, 1, 1, 1, 1)
		else
			GameTooltip:AddDoubleLine("Total", format("%.1fkb", totalKB), 1, 1, 1, 1, 1, 1)
		end

		local usedKB = GetAddOnMemoryUsage("XPerl_Options")
		if ((usedKB or 0) > 0) then
			GameTooltip:AddDoubleLine(" XPerl_Options", format("%.1fkb", usedKB), 0.5, 0.5, 0.5, 0.5, 0.5, 0.5)
		end

		if (totalCPU > 0) then
			GameTooltip:AddDoubleLine(" XPerl CPU Usage Comparison", format("%.2f%%", 100 * (totalCPU / allAddonsCPU)), 0.5, 0.5, 1, 0.5, 0.5, 1)
		end
	end

	GameTooltip:Show()
	GameTooltip.updateTooltip = 1
end


-- XPerl_SetManaBarType
local ManaColours = {		-- "mana", "rage", "focus", "energy", "runes"}
...
--[[	if (conf.tooltip.xperlInfo and XPerl_GetUsage and not strmatch(unitid, "pet")) then
		local xpUsage = XPerl_GetUsage(UnitName(unitid), unitid)
		if (xpUsage) then
...
function XPerl_ToolTip_AddBuffDuration(self, partyid, buffID)
	if (XPerl_Raid_AddBuffDuration) then
		XPerl_Raid_AddBuffDuration(self, partyid, buffID)
	end
end

-- XPerl_ToolTip_AddBuffDuration
function XPerl_ToolTip_AddDebuffDuration(self, partyid, index, cureCast)
	if (XPerl_Raid_AddDebuffDuration) then
		XPerl_Raid_AddDebuffDuration(self, partyid, index, cureCast)
	end
end
...
-- XPerl_HasBuff(unit, buffName)
function XPerl_HasBuff(unit, buffName)
	for i = 1,40 do
...
function XPerl_DebufHighlightInit()
	...
	elseif (playerClass == "SHAMAN") then
		getShow = function(Curses)
			...
			return Curses.Poison or Curses.Disease or show
		end
...
local BuffExceptions = {
	...
	PRIEST = {Spell_Nature_ResistNature = true, Spell_Nature_Rejuvenation = true, Spell_Holy_PrayerOfMendingtga = true},
	...
	SHAMAN = {Spell_Nature_ManaRegenTotem = true, Spell_Nature_UndyingStrength = true, Spell_Nature_SkinOfEarth = true,
		Spell_Nature_GroundingTotem = true, Spell_Nature_InvisibilityTotem = true, Spell_Nature_ManaRegenTotem = true,
		Spell_Nature_StoneSkinTotem = true, Spell_Nature_SlowingTotem = true, Spell_Nature_EarthBindTotem = true,
		Spell_Nature_HealingWay = true, Spell_Nature_NatureResistanceTotem = true, Spell_FrostResistanceTotem_01 = true,
		Spell_FireResistanceTotem_01 = true, Spell_Nature_Brilliance = true, Spell_Nature_EarthBind = true,
		INV_Spear_04 = true, Spell_Nature_HealingWay = true},
	PALADIN = {Spell_Holy_DevotionAura = true, Spell_Holy_AuraOfLight = true, Spell_Holy_MindSooth = true,
		Spell_Shadow_SealOfKings = true, Spell_Holy_MindVision = true, Spell_Frost_WizardMark = true,
		Spell_Fire_SealOfFire = true, Spell_Holy_CrusaderAura = true},
}

local DebuffExceptions = {
	ALL = {INV_Misc_Bandage_08 = true},		-- Recently Bandaged
	PRIEST = {Spell_Holy_AshesToAshes = true},	-- Weakened Soul
	PALADIN = {Spell_Holy_RemoveCurse = true}	-- Forbearance
}

--local SeasonalDebuffs = {				-- TODO Ignore these option
--	INV_Misc_Branch_01,				-- Mistletoe
--}

-- BuffException
--local showInfo
local function BuffException(unit, index, flag, func, exceptions)
	local name, rank, buff, count, debuffType, dur, max
	if (not flag or flag == 0 or flag == "0") then
		-- Not filtered, just return it
		name, rank, buff, count, debuffType, dur, max = func(unit, index)
		return name, rank, buff, count, debuffType, dur, max, index
	end

	name, rank, buff, count, debuffType, dur, max = func(unit, index, 1)
	if (buff) then
		-- We need the index of the buff unfiltered later for tooltips
		for i = 1,40 do
			local name1, rank1, buff1, count1, debuffType1, dur1, max1 = func(unit, i)
			if (not name1) then
				break
			end
			if (name == name1 and rank == rank1 and buff == buff1 and count == count1) then
				index = i
				break
			end
		end
	...
	for i = 1,40 do
		name, rank, buff, count, debuffType, dur, max = func(unit, i)
		if (not buff) then
			break
		end
		...
		if (strsub(buff, 1, 16) == "Interface\\Icons\\") then
			...
			if (classExceptions) then
				good = classExceptions[test]
			end
			if (not good and allExceptions) then
				good = allExceptions[test]
			end
			...
			if (good) then
				...
				if (foundValid + normalBuffFilterCount == index) then
					return name, rank, buff, count, debuffType, dur, max, i
				end
			end
		end
	end
...
-- Fix the odd difference in return values introduced with WoW 2.1.0
-- UnitBuff returns:   Name, Rank, Buff, Count, Duration, MaxDuration
-- UnitDebuff returns: Name, Rank, Buff, Count, debuffType, Duration, MaxDuration
local function XUnitBuff(unit, num, cureCast)
	local name, rank, buff, count, dur, max = UnitBuff(unit, num, cureCast)
	return name, rank, buff, count, "", dur, max
end

-- XPerl_UnitBuff
function XPerl_UnitBuff(unit, index, flag)
	return BuffException(unit, index, flag, XUnitBuff, BuffExceptions)
end
...
function XPerl_TooltipSetUnitBuff(self, unit, ind, flag)
	local name, rank, buff, count, _, dur, max, index = BuffException(unit, ind, flag, UnitBuff, BuffExceptions)
...
function XPerl_TooltipSetUnitDebuff(self, unit, ind, flag)
	local name, rank, buff, count, debuffType, dur, max, index = BuffException(unit, ind, flag, UnitDebuff, DebuffExceptions)
	if (name and index) then
...
local colourList = {
	...
	[XPERL_LOC_CLASS_WARRIORFEM or XPERL_LOC_CLASS_WARRIOR]	= "WARRIOR",
	[XPERL_LOC_CLASS_MAGEFEM or XPERL_LOC_CLASS_MAGE]		= "MAGE",
	[XPERL_LOC_CLASS_ROGUEFEM or XPERL_LOC_CLASS_ROGUE]		= "ROGUE",
	[XPERL_LOC_CLASS_DRUIDFEM or XPERL_LOC_CLASS_DRUID]		= "DRUID",
	[XPERL_LOC_CLASS_HUNTERFEM or XPERL_LOC_CLASS_HUNTER]	= "HUNTER",
	[XPERL_LOC_CLASS_SHAMANFEM or XPERL_LOC_CLASS_SHAMAN]	= "SHAMAN",
	[XPERL_LOC_CLASS_PRIESTFEM or XPERL_LOC_CLASS_PRIEST]	= "PRIEST",
	[XPERL_LOC_CLASS_WARLOCKFEM or XPERL_LOC_CLASS_WARLOCK] = "WARLOCK",
	[XPERL_LOC_CLASS_PALADINFEM or XPERL_LOC_CLASS_PALADIN]	= "PALADIN",
}
...
function XPerl_SecureUnitButton_OnLoad(self, unit, menufunc)
...
function XPerl_ShowGenericMenu(self, unit, button, anchor)
	HideDropDownMenu(1)
	...
	if (XPerl_DropDown.unit) then
		XPerl_DropDown.id = strmatch(XPerl_DropDown.unit or "", "(%d+)")
...
--ChatFrame7:AddMessage("Generic menu for "..tostring(unit).." anchor: "..tostring(anchor))

		ToggleDropDownMenu(1, nil, XPerl_DropDown, anchor or "cursor", 0, 0)
	end
end
...
function XPerl_GetBuffButton(self, buffnum, debuff, createIfAbsent)
	debuff = (debuff or 0)
...
function XPerl_CooldownFrame_SetTimer(self, start, duration, enable)
	if ( start > 0 and duration > 0 and enable > 0) then
		self:SetCooldown(start, duration)
		self.endTime = start + duration

		if (conf.buffs.countdown) then
	...
	if (UnitExists(partyid)) then
		...
		if (self.conf.buffs.enable and maxBuffs and maxBuffs > 0) then
			...
			for buffnum = 1,maxBuffs do
				local name, rank, buff, count, duration, timeLeft
				if (partyid == "player") then
					local buffIndex, untilCancelled = GetPlayerBuff(buffnum, "HELPFUL")
					if (buffIndex > 0) then
						name, rank = GetPlayerBuffName(buffIndex)
						if (name) then
							buff = GetPlayerBuffTexture(buffIndex)
							count = GetPlayerBuffApplications(buffIndex)
							timeLeft = GetPlayerBuffTimeLeft(buffIndex)
							duration = timeLeft

							if (conf.buffs.countdown) then
								-- We don't get full time duration from the API, but for our own cast buffs we can from UnitBuff
								local match = name.."_"..rank.."_"..buff
								for k = 1,40 do
									local name2, rank2, buff2, count2, duration2, timeLeft2 = UnitBuff(partyid, k)
									if (not name2) then
										break
									end
									local match2 = name2.."_"..rank2.."_"..buff2
									if (match == match2) then
										duration = duration2
										break
									end
								end
					...
					local _
					name, rank, buff, count, _, duration, timeLeft = XPerl_UnitBuff(partyid, buffnum, castableOnly)
				end

				if (buff) then
					local button = XPerl_GetBuffButton(self, buffnum, 0, true)
					button:SetAlpha(1)
					...
					if (button.cooldown) then
						if (duration and conf.buffs.cooldown) then
							local start = GetTime() - (duration - timeLeft)
							XPerl_CooldownFrame_SetTimer(button.cooldown, start, duration, 1)
						else
					...
					if (self.conf.buffs.big and duration) then
		...
		if (self.conf.debuffs.enable and maxDebuffs and maxDebuffs > 0) then
			...
			for buffnum = 1,maxDebuffs do
				--local name, rank, debuff, debuffApplications, debuffType, duration, timeLeft = UnitDebuff(partyid, buffnum, curableOnly)

				local name, rank, debuff, debuffApplications, debuffType, duration, timeLeft
				if (partyid == "player") then
					local buffIndex, untilCancelled = GetPlayerBuff(buffnum, "HARMFUL")
					if (buffIndex > 0) then
						name, rank = GetPlayerBuffName(buffIndex)
						if (name) then
							debuff = GetPlayerBuffTexture(buffIndex)
							debuffApplications = GetPlayerBuffApplications(buffIndex)
							debuffType = GetPlayerBuffDispelType(buffIndex)
							timeLeft = GetPlayerBuffTimeLeft(buffIndex)
							duration = timeLeft

							if (conf.buffs.countdown) then
								local match = format("%s_%s_%s", name, rank, debuff)
								for k = 1,40 do
									local name2, rank2, debuff2, debuffApplications2, debuffType2, duration2, timeLeft2 = UnitDebuff(partyid, k)
									if (not name2) then
										break
									end
									local match2 = format("%s_%s_%s", name2, rank2, debuff2)
									if (match == match2) then
										duration = duration2
										break
									end
					...
					name, rank, debuff, debuffApplications, debuffType, duration, timeLeft = XPerl_UnitDebuff(partyid, buffnum, curableOnly)
				end

				if (debuff) then
					local button = XPerl_GetBuffButton(self, buffnum, 1, true)
					button:SetAlpha(1)
					...
					if (self.conf.debuffs.big and duration) then
...
local function XPerlSetPortrait3D(self, argUnit)
	...
	-- Workaround for the Dragonmaw illusion. They set the cameras wrong. 1 is 0, 0 is 1
	if (strfind(UnitName(argUnit), "Dragonmaw Peon") or strfind(UnitName(argUnit), "Dragonmaw Transporter") or XPerl_HasBuff(argUnit, "Dragonmaw Illusion")) then
		self:SetCamera(1)
	else
		self:SetCamera(0)
	end
...
function XPerl_Unit_SetBuffTooltip(self)
	if (conf.tooltip.enableBuffs and XPerl_TooltipModiferPressed(true)) then
		if (not conf.tooltip.buffHideInCombat or not InCombatLockdown()) then
			...
			local partyid = frame.partyid		--:GetAttribute("unit")
			if (partyid) then
				GameTooltip:SetOwner(self, "ANCHOR_BOTTOMRIGHT",0,0)
				XPerl_TooltipSetUnitBuff(GameTooltip, partyid, self:GetID(), frame.conf.buffs.castable)
				XPerl_ToolTip_AddBuffDuration(self, partyid, self:GetID())
			end
		end
	end
end
...
function XPerl_Unit_SetDeBuffTooltip(self)
	if (conf.tooltip.enableBuffs and XPerl_TooltipModiferPressed(true)) then
		if (not conf.tooltip.hideInCombat or not InCombatLockdown()) then
			...
			if (partyid) then
				GameTooltip:SetOwner(self, "ANCHOR_BOTTOMRIGHT",0,0)
				XPerl_TooltipSetUnitDebuff(GameTooltip, partyid, self:GetID(), frame.conf.debuffs.curable)
				XPerl_ToolTip_AddDebuffDuration(self, partyid, self:GetID(), frame.conf.debuffs.curable)
			end
		end
	end
end
	...
	if (HealComm) then
		...
		local function UpdateIncomingHeals(healerFullName, healSize, ...)
			for i = 1,select("#",...) do
				...
				if (healSize and UnitIsUnit("player", healerFullName) and myHeals[guid]) then
				...
				if (XPerl_Target_SetHealth and guid == focusGUID) then
					XPerl_Target_SetHealth(XPerl_Focus)
				end
				if (XPerl_Target_SetHealth and guid == targettargetGUID) then
		...
		function XPerl_ExpectedUnitHealth(unit)
			...
			local incoming = HealComm:UnitIncomingHealGet(unit, GetTime() + 100.0) or 0
			incoming = incoming + (myHeals[UnitGUID(unit)] or 0)
			if (incoming > 0) then
				local effective = incoming * HealComm:UnitHealModifierGet(unit)
		...
		HealComm.RegisterCallback("XPerlHealCommCallback", "HealComm_DirectHealStart", xperl_HealComm_DirectHealStart)
		HealComm.RegisterCallback("XPerlHealCommCallback", "HealComm_DirectHealStop", xperl_HealComm_DirectHealStop)
		HealComm.RegisterCallback("XPerlHealCommCallback", "HealComm_DirectHealDelayed", xperl_HealComm_DirectHealDelayed)
		HealComm.RegisterCallback("XPerlHealCommCallback", "HealComm_HealModifierUpdate", xperl_HealComm_HealModifierUpdate)
	end
end
